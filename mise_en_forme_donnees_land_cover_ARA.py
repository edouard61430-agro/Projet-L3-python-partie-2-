...
def plot_land_cover(annee: int = 0):
    '''
    ...
    # Crée une liste contenant les années à étudier
    annees = [1990, 2000, 2006, 2012] if annee == 0 else [annee]

    liste_surfaces = []
    # Pour toutes les années sélectionnées,
    for an in annees:
        # on importe le dataframe avec la fonction import_land_use()
        code_annee = str(an)[-2:]
        land_use = import_land_use('ARA', code_annee)
